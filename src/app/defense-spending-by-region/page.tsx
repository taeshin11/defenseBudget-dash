...
export default function DefenseSpendingByRegionPage() {
    ...
    <div className="space-y-10">
      ...
      {regionData.map((r) => (
        <section key={r.name} className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Link
              href={`/compare?countries=${r.countries.slice(0, 8).map((c) => c.code).join(",")}&metric=${r.compareMetric}`}
      ...
      <section className="rounded-xl bg-accent-navy-light p-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-4">
          ...
          <Link href="/compare" className="inline-flex items-center rounded-xl border border-border bg-white px-5 py-3 text-sm font-medium text-text-secondary shadow-sm hover:border-accent-navy hover:text-accent-navy transition-all">
            Custom Comparison
          </Link>
        </div>
      </section>
