...
export default async function TopMilitarySpendersPage({ params }: { params: Promise<{ lang: string }> }) {
    ...
    <div className="space-y-10">
          ...
          <div key={c.code} className="rounded-xl bg-white p-6 shadow-md transition-shadow duration-300 hover:shadow-lg">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex items-center gap-3 sm:w-48">
                ...
                <Link href={`${prefix}/country/${c.code.toLowerCase()}`} className="font-heading text-lg font-bold text-text-primary hover:text-accent-navy transition-colors">
                  {c.name}
                </Link>
              </div>
      ...
      <section className="rounded-xl bg-accent-navy-light p-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-4">
          ...
          <Link href={`${prefix}/compare?countries=US,CN,RU,IN,SA,GB,DE,FR`} className="inline-flex items-center rounded-xl border border-border bg-white px-5 py-3 text-sm font-medium text-text-secondary shadow-sm hover:border-accent-navy hover:text-accent-navy transition-all">
            Compare Top 8
          </Link>
        </div>
      </section>
